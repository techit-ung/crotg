...
- **Run (TUI)**: `go run ./cmd/reviewer` (supports flags: `--base main --branch feature/foo`)
- **Test All**: `go test ./...`
...
- **Tests**:
  ...
  - Target coverage: ≥80% for protocol/schema packages (`internal/review`, `internal/publish`).
...
- `internal/app`: Main [Bubble Tea](https://github.com/charmbracelet/bubbletea) model and state machine (Wizard vs. Dashboard tabs).
